...
このレクチャーのコマンドは、すべて `sections/03-build-app/02-d1/` フォルダの中で実行します。コマンドを打つ前に、今いる場所を確認しましょう。
...
表示された末尾が `.../sections/03-build-app/02-d1` になっていれば OK です。違っていたら、リポジトリのルート（`cloudflare-introduction-handson/`）から次で移動します（macOS / Linux / Windows 共通）。

```bash
cd sections/03-build-app/02-d1
```
:::
...
`.../sections/03-build-app/02-d1` にいなければ、次で移動します（macOS / Linux / Windows 共通）。
