...
まず、このセクション用の設定ファイル `wrangler.jsonc` を用意します。リポジトリにはテンプレートの
`wrangler.example.jsonc` だけが入っているので、**同じフォルダ上でこのファイルを複製し、複製した方の
名前を `wrangler.jsonc` に変更**します（各ステップで毎回この作業をして、自分専用のアプリを新しく
作ります）。エクスプローラーや VSCode でファイルをコピー＆ペーストし、名前を付け替えるだけで大丈夫です。

複製した [wrangler.jsonc](./wrangler.jsonc) を開き、`name` の「あなたの名前」の部分を自分用に
書き換えます（例: `hitokoto-tanaka-01-workers`）。他の人と同じ名前だと公開時にぶつかるので、必ず
自分だけの名前にしてください。`wrangler.jsonc` は Git 管理外なので、あなたの名前入りの設定は
コミットされません。
...
前章 [Pages](../../01-publish/02-pages/LECTURE.md) では、公開したものを **ダッシュボード（画面）** から
削除しました。ここでは同じ「削除」を **CLI（コマンド）** でやってみます。慣れると画面を開くより速く、
...
:::notice
Worker を消しても、前章で公開したフロント（Pages）は別のプロジェクトなので残ります。Pages も
消したい場合は、前章の手順（ダッシュボード）で削除してください。
:::
