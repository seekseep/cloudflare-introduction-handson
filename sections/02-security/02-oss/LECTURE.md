...
攻撃者は乗っ取ったアカウントを使い、多くの利用者がいるパッケージに悪意のあるプログラムを公開しました。これらのパッケージは週に20億回以上ダウンロードされており、更新した開発者の環境へ不正なコードが配布される危険がありました。悪意のあるコードは、暗号資産の送金先を利用者に気付かれないよう書き換えるものでした。

さらに同じ月には、「Shai-Hulud」と呼ばれる自己増殖型ワームも見つかりました。このワームは、感染した開発者の認証情報を盗み、その開発者が管理する別のパッケージにも自分自身を公開して感染を広げる仕組みでした。短期間で数百ものパッケージが影響を受ける大規模な被害となりました。

この事件は、有名なライブラリでも安全とは限らないことを示しました。ソフトウェアの安全性はプログラムの品質だけでなく、開発者のアカウント管理や認証情報の保護にも左右されます。そのため現在では、多要素認証の利用や公開権限の管理、依存パッケージの監視などが重要な対策となっています。
