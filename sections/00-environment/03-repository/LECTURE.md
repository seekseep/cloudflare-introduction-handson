...
**git を使う方法** と **Web から ZIP でダウンロードする方法** の 2 通りがあります。どちらでも
中身は同じなので、やりやすい方で構いません。

![git clone または ZIP ダウンロードで手元にリポジトリを取得し、VSCode で開くまでの流れ](./images/01-get-repo-flow.svg)

<!-- genfig: リポジトリ入手の流れを描く。左に「GitHub リポジトリ(☁️)」を SOURCE として置く。そこから2本の経路を右へ分岐(SPLITTING)：上の経路ラベル「方法A: git clone」、下の経路ラベル「方法B: ZIP をダウンロード」。2本は中央の「手元のフォルダ cloudflare-introduction-handson(📁)」に合流(MERGING)。そこから右へ connector を引き、ラベル「VSCode で開く」、ゴールに「VSCode(💻)」を置く。左→右の一方向フロー。イメージスキーマ = SOURCE-PATH-GOAL + SPLITTING(2経路に分かれる) + MERGING(同じフォルダに合流)。絵文字割当: GitHubリポジトリ=☁️ / 手元のフォルダ=📁 / VSCode=💻。 -->
*図: git clone か ZIP ダウンロードのどちらかで手元にフォルダを用意し、それを VSCode で開く。*

## どちらの方法を選ぶか

- **git を使ったことがある / これから使いたい人** → 方法 A（`git clone`）。あとから教材が更新されても
  `git pull` で追従できます。
- **git をまだ使っていない人 / とにかく早く始めたい人** → 方法 B（ZIP ダウンロード）。git のインストールは
  不要です。

:::notice
どちらでも進められます。迷ったら、まずは手軽な **方法 B（ZIP）** で始めて構いません。
:::

## 方法 A: git clone で入手する
...
手元にできたフォルダを [VSCode](../02-tools/LECTURE.md) で開きます。方法はいくつかあります。
...
開いたら、VSCode 内でターミナルを開いておくと以降の作業がスムーズです。
``Ctrl + ` ``（バッククォート）でターミナルが開きます。今どこにいるかは `pwd`（Windows は `cd`）で
確認できます。ここがリポジトリのルート（`cloudflare-introduction-handson/`）になっていれば準備完了です。
