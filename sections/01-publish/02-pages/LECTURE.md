...
:::notice[今いるフォルダを確認]
`cd` のあと、本当にこのフォルダに移動できたかを確認しましょう。
...
表示された末尾が `.../sections/01-publish/02-pages` になっていれば OK です。VSCode では ``Ctrl + ` ``（バッククォート）で開いたターミナルはリポジトリのルートにいることが多いので、その場合は上の `cd sections/01-publish/02-pages` を先に実行してください。
:::
...
ターミナルを新しく開くと、別の場所（多くはリポジトリのルート）から始まることがあります。デプロイの前に、今いる場所を確認しましょう。
...
`.../sections/01-publish/02-pages` にいなければ、次で移動します（macOS / Linux / Windows 共通）。
