...
<!-- genfig: 中央に「☁️ Cloudflare（本番環境）」を置き、左右から2つの操作元が向かう対比図。左=「💻 開発PC：wrangler(CLI)」から Cloudflare へ「作る・反映する（deploy）」の濃い矢印（往路）。右=「🌐 ブラウザ：ダッシュボード(GUI)」と Cloudflare の間を「状態を確認する」の薄い双方向矢印。登場要素 = 💻(開発PC/CLI), 🌐(ブラウザ/GUIダッシュボード), ☁️(Cloudflare本番環境)。関係はすべて矢印ラベルで表現しノード化しない。イメージスキーマ = CENTER-PERIPHERY（中央のCloudflareに2つの操作経路が集まる）+ SOURCE-PATH-GOAL（操作→Cloudflare）。絵文字割当: 開発PC/CLI=💻 1f4bb, ブラウザ/GUI=🌐 1f310, Cloudflare本番=☁️ 2601。 -->
